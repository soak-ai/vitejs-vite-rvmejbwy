// Heartfelt — Locked Version
// Based on App_perfect_gw_empty_state_49_ with 4 targeted fixes:
// FIX 1: burstPop multiplier 3.5 → 2  (confetti visible on mobile)
// FIX 2: cat-card hover wrapped in @media(hover:hover)  (no ghost tap on mobile)
// FIX 3: CatCard default borderColor uses cat.accent+"38"  (color visible without hover)
// FIX 4: wallMessages persisted in localStorage  (cards survive refresh/sleep on mobile)
//        Initial state is [] so new visitors always see the beautiful empty state
...
const Styles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-primary { padding: 11px 24px; font-size: 14px; background: var(--ink); color: var(--white); box-shadow: 0 2px 8px rgba(24,18,14,.18); }
    .btn-primary:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(24,18,14,.24); }
    .btn-secondary { padding: 10px 22px; font-size: 14px; background: var(--white); color: var(--ink); border: 1.5px solid var(--sand-200) !important; }
    .btn-secondary:hover:not(:disabled) { border-color: var(--ink) !important; transform: translateY(-1px); }
    .btn-ghost { padding: 9px 16px; font-size: 13px; background: transparent; color: var(--ink-3); }
    .btn-ghost:hover:not(:disabled) { background: var(--sand-100); color: var(--ink-2); }
    .btn-accent { padding: 11px 24px; font-size: 14px; color: var(--white); box-shadow: 0 2px 8px rgba(0,0,0,.08); }
    ...
    /* FIX 2: hover only fires on real pointer devices, not touch */
    @media (hover: hover) {
    ...
    .shimmer-card-glow { background-image:linear-gradient(90deg,transparent 0%,rgba(255,255,255,.55) 50%,transparent 100%) !important; background-size:400px 100% !important; animation:shimmerSweep 2.8s linear infinite !important; }
    ...
    .sticky-note { transition: transform .2s ease, box-shadow .2s ease; transform: rotate(var(--r, 0deg)); }
    .sticky-note-hover:hover { transform: rotate(0deg) translateY(-4px); box-shadow: 0 12px 32px rgba(0,0,0,.12) !important; }

    @media (max-width: 900px) { .compose-grid { grid-template-columns: 1fr !important; } .compose-preview { position: static !important; } }
    ...
    .step-dot { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Lato', sans-serif; font-size: 11px; transition: all .3s ease; }
    .sh-btn { display: inline-flex; align-items: center; gap: 6px; padding: 9px 14px; border-radius: 10px; font-family: 'Lato', sans-serif; font-size: 13px; font-weight: 500; cursor: pointer; border: 1.5px solid transparent; transition: transform .16s, box-shadow .16s; }
    ...
    /* FIX 1: multiplier 3.5 → 2 so confetti stays visible on phone screens */
...
function PolishButton({ message, onPolished, accent }) {
  ...
  const polish = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:300, messages:[{ role:"user", content:`You are helping someone write a heartfelt personal message. Take this message and make it sound more beautiful and emotionally resonant — keep the exact same meaning, the same personal voice, and the same feeling. Replace generic words with more specific, warm ones. Make it feel like it came from the heart. Do NOT add new ideas, do NOT make it longer or shorter than necessary, do NOT use clichés. Return ONLY the improved message, nothing else.\n\nOriginal message: ${message}` }] })
      });
      const data = await res.json();
      const polished = data?.content?.[0]?.text?.trim();
      if (polished && polished !== message) { onPolished(polished); setState("done"); setTimeout(() => setState("idle"), 2500); }
      else { setState("done"); setTimeout(() => setState("idle"), 2000); }
  ...
  return (
    <button onClick={polish} disabled={state==="loading" || !message.trim()}
      ...
      onMouseOver={e=>{ if(state==="idle"&&message.trim()){ e.currentTarget.style.borderColor=accent; e.currentTarget.style.color=accent; }}}
      onMouseOut={e=>{ if(state==="idle"){ e.currentTarget.style.borderColor="var(--sand-200)"; e.currentTarget.style.color="var(--ink-3)"; }}}>
      ...
      {state==="idle" && <span style={{ fontSize:11 }}>✦</span>}
...
function AISuggest({ catId, onUse }) {
  ...
  const suggest = useCallback(async () => {
    ...
    setUsed(u => [...u.slice(-4), pick]); setText(pick); setLoading(false);
  }, [catId, used]);
  if (!shown) return (
    <button onClick={suggest} style={{ display:"inline-flex", alignItems:"center", gap:7, padding:"9px 16px", borderRadius:999, border:"1.5px solid var(--sand-200)", background:"rgba(255,255,255,.75)", fontFamily:"'Lato', sans-serif", fontSize:13, fontWeight:500, color:"var(--ink-3)", cursor:"pointer", transition:"all .18s" }}
  ...
  return (
    <div style={{ borderRadius:12, border:"1.5px solid var(--sand-200)", background:"rgba(255,255,255,.88)", padding:"14px 16px" }}>
      <div style={{ fontSize:11, color:"var(--ink-3)", marginBottom:8 }}>✦ heartfelt suggestion</div>
      {loading
        ? <div style={{ display:"flex", gap:6, alignItems:"center", justifyContent:"center", height:40 }}>
        ...
        : <>
            ...
            <div style={{ display:"flex", gap:8, justifyContent:"center" }}>
              <button onClick={() => onUse(text)} style={{ padding:"7px 14px", borderRadius:999, border:`1.5px solid var(--ink)`, background:"var(--ink)", color:"#fff", fontFamily:"'Lato', sans-serif", fontSize:13, cursor:"pointer", transition:"all .18s" }} onMouseOver={e=>{ e.currentTarget.style.opacity=".88"; }} onMouseOut={e=> { e.currentTarget.style.opacity="1"; }}>Use this</button>
              <button onClick={suggest} style={{ padding:"7px 14px", borderRadius:999, border:"1.5px solid var(--sand-200)", background:"transparent", color:"var(--ink-3)", fontFamily:"'Lato', sans-serif", fontSize:13, cursor:"pointer", display:"inline-flex", alignItems:"center", gap:4, transition:"border-color .18s, color .18s" }} onMouseOver={e=>{ e.currentTarget.style.borderColor="var(--sand-300)"; e.currentTarget.style.color="var(--ink-2)"; }} onMouseOut={e=> { e.currentTarget.style.borderColor="var(--sand-200)"; e.currentTarget.style.color="var(--ink-3)"; }}>Another <span style={{ display:"inline-block", fontSize:12 }}>✦</span></button>
            </div>
          </>}
    </div>
  );
...
function ShareCTA({ accent, recip, onHome }) {
  ...
  const shareBtn = { width:"100%", padding:"9px 16px", borderRadius:12, fontFamily:"'Lato', sans-serif", fontSize:13, fontWeight:400, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:10, transition:"background .2s, border-color .2s", background:"transparent" };
  const icon = (em) => <span style={{ fontSize:20, lineHeight:1, flexShrink:0 }}>{em}</span>;
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:9, width:"100%" }}>
      <button style={{ ...shareBtn, color:"#1a7a42", border:"1.5px solid #a8dbb8" }} onClick={() => window.open("https://wa.me/?text="+encodeURIComponent("I made something for you 💛 "+link),"_blank")} onMouseOver={e=>{ e.currentTarget.style.background="#e6f7ed"; e.currentTarget.style.borderColor="transparent"; }} onMouseOut={e=> { e.currentTarget.style.background="transparent"; e.currentTarget.style.borderColor="#a8dbb8"; }}>{icon("💬")} Send via WhatsApp</button>
      <button style={{ ...shareBtn, color:"#b5467a", border:"1.5px solid #f0b8d4" }} onClick={() => { copy(); alert("Link copied! Paste it in your Instagram DM or Story."); }} onMouseOver={e=>{ e.currentTarget.style.background="#fce8f2"; e.currentTarget.style.borderColor="transparent"; }} onMouseOut={e=> { e.currentTarget.style.background="transparent"; e.currentTarget.style.borderColor="#f0b8d4"; }}>{icon("📷")} Share on Instagram</button>
      <button style={{ ...shareBtn, color:copied?"#2e7d32":"#2563a8", border:`1.5px solid ${copied?"#a8dbb8":"#a8c4e8"}` }} onClick={copy} onMouseOver={e=>{ e.currentTarget.style.background=copied?"#e8f5e9":"#e8f0fb"; e.currentTarget.style.borderColor="transparent"; }} onMouseOut={e=> { e.currentTarget.style.background="transparent"; e.currentTarget.style.borderColor=copied?"#a8dbb8":"#a8c4e8"; }}>{icon(copied?"✓":"🔗")} {copied ? "Copied! Paste it anywhere." : "Copy link"}</button>
      <div style={{ height:1, background:"var(--sand-200)", margin:"6px 0" }}/>
      <div style={{ display:"flex", justifyContent:"center", marginTop:6 }}>
        <button style={{ padding:"7px 18px", borderRadius:999, fontFamily:"'Lato', sans-serif", fontSize:12, color:"var(--ink-3)", cursor:"pointer", display:"inline-flex", alignItems:"center", gap:5, border:"1.5px solid var(--sand-300)", background:"transparent", transition:"border-color .18s, color .18s" }} onClick={onHome} onMouseOver={e=>{ e.currentTarget.style.borderColor="var(--ink)"; e.currentTarget.style.color="var(--ink)"; }} onMouseOut={e=> { e.currentTarget.style.borderColor="var(--sand-300)"; e.currentTarget.style.color="var(--ink-3)"; }}>✦ Send another <span style={{ display:"inline-block", animation:"arrowPulse 1.4s ease-in-out infinite", fontSize:12 }}>→</span></button>
      </div>
    </div>
  );
}

function SentPage({ cat, formData, onHome }) {
  ...
  return (
    <div className="anim-popin" style={{ maxWidth:420, margin:"0 auto", padding:"64px 24px", textAlign:"center", position:"relative" }}>
      {burst && positions.map((p,i) => (
        <div key={i} style={{ position:"fixed", top:"50%", left:"50%", fontSize:p.s||22, lineHeight:1, "--bx":`${p.bx}px`, "--by":`${p.by}px`, animation:`burstPop 1.4s cubic-bezier(.22,1,.36,1) ${p.d*.8}s both`, pointerEvents:"none", zIndex:50 }}>{emojis[i % emojis.length]}</div>
      ))}
      <div style={{ fontSize:52, marginBottom:14, animation:"sentBounce .5s cubic-bezier(.34,1.56,.64,1) both", lineHeight:1 }}>💌</div>
      <h2 style={{ fontFamily:"'Lora',serif", fontStyle:"italic", fontSize:22, color:"var(--ink)", marginBottom:8, lineHeight:1.2 }}>Your card is sealed <span style={{display:"inline-block",animation:"shimmerStar 1.8s ease-in-out infinite"}}>✦</span></h2>
      <p style={{ fontSize:14, color:"var(--ink-3)", lineHeight:1.7, marginBottom:6 }}>Pick how you want to share it with <strong style={{ color:"var(--ink)" }}>{recip}</strong>.</p>
...
function GratitudeWall({ wallMessages, onHome }) {
  ...
  return (
    <div className="anim-fadeup" style={{ maxWidth:1080, margin:"0 auto", padding:"56px 24px 80px" }}>
      ...
      {/* ── EMPTY STATE — shown to brand new visitors ── */}
      {isEmpty && (
        <div style={{ maxWidth:520, margin:"0 auto", textAlign:"center" }}>
          <div style={{ fontSize:48, marginBottom:20, lineHeight:1 }}>💌</div>
          <h2 style={{ fontFamily:"'Lora',serif", fontStyle:"italic", fontSize:"clamp(18px,3vw,24px)", color:"var(--ink)", marginBottom:10, lineHeight:1.3 }}>Be the first to bond.</h2>
          <p style={{ fontSize:14, color:"var(--ink-3)", lineHeight:1.7, marginBottom:24, maxWidth:340, margin:"0 auto 24px" }}>Say the thing you've been holding back.</p>
          <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center", marginBottom:28 }}>
            {[{ icon:"🙏", text:"Thank someone" },{ icon:"❤️", text:"Say you care" },{ icon:"✦", text:"Celebrate them" }].map(({ icon, text }) => (
          ...
          <button onClick={onHome}
            ...
            Send a card <span style={{ display:"inline-block", animation:"arrowPulse 1.4s ease-in-out infinite" }}>→</span>
          </button>
          <p style={{ fontSize:11, color:"var(--ink-3)", marginTop:14, fontStyle:"italic", opacity:.65 }}>Anonymous · no account needed</p>
        </div>
      )}

      {/* ── CARDS — flat grid, works on all screen sizes ── */}
      {!isEmpty && (
          ...
          {wallMessages.map((item, i) => {
            ...
            return (
              <div key={item.id} className={`masonry-item sticky-${mod} sticky-note`}
                style={{ background:item.color, borderRadius:16, padding:24, border:`1px solid ${cat?.accent||"#eee"}33`, boxShadow:"0 4px 16px rgba(0,0,0,.07)", animation:`stickyPop .4s ease both`, animationDelay:`${i*.05}s`, position:"relative", cursor:"default", userSelect:"none" }}>
                {isNew && <div style={{ marginBottom:8 }}><span style={{ fontSize:10, background:cat?.accent||"#888", color:"#fff", borderRadius:999, padding:"2px 10px", fontFamily:"'Lato', sans-serif", fontWeight:500, letterSpacing:".04em", display:"inline-block" }}>just now</span></div>}
                <div style={{ fontSize:11, letterSpacing:".12em", textTransform:"uppercase", color:cat?.accent, marginBottom:12, fontFamily:"'Lato', sans-serif" }}>{cat?.glyph} {cat?.label}</div>
                <p style={{ fontSize:14, color:"var(--ink)", lineHeight:1.75, fontStyle:"italic", fontFamily:"'Lora',serif", marginBottom:12 }}>"{item.message}"</p>
              </div>
            );
          })}
        </div>
...
function CatCard({ cat, onClick }) {
  ...
  return (
    // FIX 3: default border uses cat.accent+"38" — color always visible on mobile
    <div className="cat-card" onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div className="cat-icon" style={{ width:48, height:48, borderRadius:14, background:`linear-gradient(135deg,${cat.light},${cat.accent}28)`, border:`1px solid ${cat.accent}40`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, color:cat.accent, fontWeight:700, marginBottom:14, transition:"all .22s cubic-bezier(.34,1.56,.64,1)", flexShrink:0 }}>{cat.glyph}</div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:4, fontSize:12, color:cat.accent }}>Select <span style={{ display:"inline-block", animation:"arrowPulse 1.4s ease-in-out infinite", fontSize:14 }}>→</span></div>
    </div>
  );
}
...
function ComposeStep({ cat, onPreview, onBack, draft }) {
  ...
  const swapNames = () => { setSwapAnim(true); setTimeout(() => setSwapAnim(false), 400); const tmp = sender; setSender(recipient); setRecipient(tmp); };
  ...
  return (
    <div className="anim-slidein" style={{ maxWidth:1100, margin:"0 auto", padding:"0 24px 64px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, marginBottom:36 }}>
        ...
        <div><div style={{ fontSize:15, fontWeight:600, color:"var(--ink)" }}>{cat.label}</div><div style={{ fontSize:13, color:"var(--ink-3)" }}>{cat.sub}</div></div>
      </div>
      <div className="compose-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:32, alignItems:"start" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <button onClick={swapNames} style={{ width:32, height:32, borderRadius:"50%", border:"1.5px solid var(--sand-200)", background:"rgba(255,255,255,.85)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:16, lineHeight:1, transition:"all .18s", transform: swapAnim ? "rotate(180deg) scale(1.15)" : "rotate(0deg) scale(1)", color:"var(--ink-3)" }} onMouseOver={e => { e.currentTarget.style.borderColor="var(--ink)"; e.currentTarget.style.color="var(--ink)"; }} onMouseOut={e => { e.currentTarget.style.borderColor="var(--sand-200)"; e.currentTarget.style.color="var(--ink-3)"; }}>⇅</button>
          </div>
          {[{ l:"Your name", p:"e.g. Arjun", v:sender, s:setSender },{ l:"Recipient name", p:"e.g. Priya", v:recipient, s:setRecipient }].map(({ l,p,v,s }) => (
            <div key={l}><label style={{ display:"block", fontSize:12, color:"var(--ink-3)", marginBottom:5 }}>{l}</label><input value={v} onChange={e=>s(e.target.value)} placeholder={p} type="text" {...inp(l)}/></div>
          ))}
          <div style={{ borderTop:"1px solid var(--sand-200)", paddingTop:18 }}>
            ...
            {message.trim().length > 8 && <PolishButton message={message} onPolished={setMessage} accent={cat.accent}/>}
          </div>
          <div style={{ marginTop:4 }}><AISuggest catId={cat.id} onUse={t => setMessage(t)}/></div>
          <div>
            <div style={{ fontSize:12, color:"var(--ink-3)", marginBottom:8 }}>Need a nudge?</div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {(TEMPLATES[cat.id]||[]).map((t,i) => (
                <button key={i} className="prompt-pill" onClick={() => setMessage(t)}><span style={{ opacity:.4, marginRight:5 }}>"</span>{t.slice(0,80)}{t.length>80?"…":""}<span style={{ opacity:.4 }}>"</span></button>
              ))}
            </div>
          </div>
        </div>
        <div className="compose-preview" style={{ display:"flex", flexDirection:"column", gap:16, position:"sticky", top:80 }}>
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
              {BG_OPTIONS.map(o => (<button key={o.id} onClick={() => setBgMode(o.id)} style={{ padding:"7px 14px", borderRadius:999, border:`1.5px solid ${bgMode===o.id?cat.accent:"var(--sand-200)"}`, background:bgMode===o.id?cat.accent+"18":"rgba(255,255,255,.75)", fontFamily:"'Lato', sans-serif", fontSize:13, color:bgMode===o.id?cat.accent:"var(--ink-3)", cursor:"pointer", transition:"all .18s" }}>{o.label}</button>))}
              <button onClick={() => setShimmerOn(s => !s)} style={{ padding:"7px 14px", borderRadius:999, border:`1.5px solid ${shimmerOn?cat.accent:"var(--sand-200)"}`, background:shimmerOn?cat.accent+"18":"rgba(255,255,255,.75)", fontFamily:"'Lato', sans-serif", fontSize:13, color:shimmerOn?cat.accent:"var(--ink-3)", cursor:"pointer", transition:"all .18s", display:"flex", alignItems:"center", gap:4 }}>✨ Shimmer</button>
            </div>
          </div>
          <div>
            <div style={{ fontSize:12, color:"var(--ink-3)", marginBottom:8 }}>Text style</div>
            <div style={{ display:"flex", gap:6 }}>
              {FONT_OPTIONS.map(f => (<button key={f.id} onClick={() => setFontId(f.id)} style={{ flex:1, padding:"9px 10px", borderRadius:10, border:`1.5px solid ${fontId===f.id?cat.accent:"var(--sand-200)"}`, background:fontId===f.id?cat.accent+"18":"rgba(255,255,255,.75)", cursor:"pointer", transition:"all .16s", textAlign:"left" }}><div style={{ ...f.style, fontSize:f.id==="sacramento"?24:14, color:fontId===f.id?cat.accent:"var(--ink)", marginBottom:3 }}>Aa</div><div style={{ fontSize:11, color:fontId===f.id?cat.accent:"var(--ink-3)" }}>{f.label}</div></button>))}
            </div>
          </div>
        </div>
      </div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:40, paddingTop:28, borderTop:"1px solid var(--sand-200)" }}>
        ...
        <div style={{ display:"flex", gap:12, alignItems:"center" }}>
          ...
          <button className="btn btn-accent" disabled={!canGo} onClick={() => onPreview({ sender, recipient, message, bgMode, shimmerOn, fontId })} style={{ background:canGo?cat.accent:"var(--sand-300)" }}>Preview card →</button>
        </div>
      </div>
    </div>
  );
...
export default function Heartfelt() {
  ...
  // FIX 4: localStorage persistence
  // Default is [] — new visitors see empty state, returning visitors see their cards
  ...
  const handleSend = () => {
    if (formData) {
      setWallMessages(prev => [{ id:Date.now(), cat:cat.id, message:formData.message, color:cat.light, accent:cat.accent, isNew:true, timeLabel:"just now" }, ...prev]);
    }
    setCelebrating(true);
      ...
      <nav style={{ position:"sticky", top:0, zIndex:20, background:"rgba(253,250,246,.94)", backdropFilter:"blur(18px)", borderBottom:"1px solid var(--sand-200)", height:58 }}>
        <div className="nav-wrap" style={{ maxWidth:1152, margin:"0 auto", padding:"0 32px", height:"100%", display:"flex", alignItems:"center", position:"relative" }}>
          ...
          <div style={{ position:"absolute", left:"50%", transform:"translateX(-50%)" }}>
            <button onClick={goHome} className="nav-item-press" style={{ padding:"9px 22px", borderRadius:999, border:"none", background:"var(--ink)", color:"#fff", fontFamily:"'Lato', sans-serif", fontSize:13, fontWeight:500, cursor:"pointer", transition:"box-shadow .16s, transform .12s", boxShadow:"0 2px 10px rgba(24,18,14,.18)", whiteSpace:"nowrap" }} onMouseOver={e => { e.currentTarget.style.boxShadow="0 4px 18px rgba(24,18,14,.28)"; e.currentTarget.style.transform="translateY(-1px)"; }} onMouseOut={e => { e.currentTarget.style.boxShadow="0 2px 10px rgba(24,18,14,.18)"; e.currentTarget.style.transform="translateY(0)"; }}>Send a Card</button>
          </div>
          <div style={{ flex:1, display:"flex", justifyContent:"flex-end", alignItems:"center", gap:8 }}>
            <button onClick={() => setPage("wall")} className="nav-item-press" style={{ background:"none", border:"none", fontFamily:"'Lato', sans-serif", fontSize:14, fontWeight:page==="wall"?600:400, color:"var(--ink-3)", cursor:"pointer", transition:"color .16s", position:"relative", paddingBottom:4 }} onMouseOver={e=>{ if(page!=="wall") e.currentTarget.style.color="var(--ink-2)"; }} onMouseOut={e=>{ if(page!=="wall") e.currentTarget.style.color="var(--ink-3)"; }}>
              Gratitude Wall
        ...
        {page !== "wall" && <>
          {page === "home" && (
            <div className="anim-fadeup" style={{ maxWidth:1080, margin:"0 auto", padding:"60px 24px 80px" }}>
              <div style={{ textAlign:"center", marginBottom:52 }}>
                ...
                <h1 style={{ fontSize:"clamp(30px,5.5vw,52px)", fontWeight:600, color:"var(--ink)", letterSpacing:"-.03em", lineHeight:1.08, marginBottom:16, fontFamily:"'Lora',serif", fontStyle:"italic" }}>Say the thing<br/><span style={{ fontStyle:"normal" }}>you keep meaning to.</span></h1>
                <p style={{ fontSize:15, color:"var(--ink-3)", lineHeight:1.7, maxWidth:400, margin:"0 auto" }}>Pick a category. Write your message. Felt, not generated.</p>
              </div>
              ...
              <div style={{ marginTop:48, paddingTop:20, borderTop:"1px solid var(--sand-200)", display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
                ...
                <span style={{ fontSize:13, color:"var(--ink-3)" }}>free · no account needed</span>
              </div>
            </div>
          )}
          {page === "compose" && cat && (
          ...
          {page === "preview" && cat && formData && (
            <div className="anim-slidein" style={{ paddingTop:44, paddingBottom:64, position:"relative", overflow:"hidden" }}>
              {celebrating && (() => {
                const catEmojis = { gratitude:["🌼","✦","🌼","✧","🌼","✦","✧","🌼","✦","🌼","✧","✦"], love:["❤️","♡","❤️","✦","♡","❤️","✧","♡","❤️","♡","✦","❤️"], birthday:["⭐","🎂","⭐","✦","⭐","🎂","✦","⭐","✦","🎂","⭐","✦"], cheer:["☀️","✦","☀️","✧","☀️","✦","◇","☀️","✦","☀️","✧","◇"], amends:["💛","🤝","💛","✦","💛","🤝","✧","💛","✦","💛","🤝","💛"], celebrate:["🏆","🎉","✦","🏆","🎉","✦","🏆","✦","🎉","🏆","✦","🎉"] };
                const emojis = catEmojis[cat.id] || catEmojis.love;
                const positions = [{bx:-80,by:-100,d:0,s:26},{bx:80,by:-100,d:.05,s:22},{bx:-115,by:-30,d:.1,s:20},{bx:115,by:-30,d:.08,s:24},{bx:-55,by:90,d:.12,s:22},{bx:55,by:90,d:.06,s:20},{bx:-135,by:35,d:.04,s:18},{bx:135,by:35,d:.09,s:26},{bx:0,by:-125,d:.02,s:24},{bx:-90,by:70,d:.11,s:20},{bx:90,by:70,d:.07,s:22},{bx:0,by:125,d:.03,s:18},{bx:-160,by:-50,d:.06,s:16},{bx:160,by:-50,d:.13,s:18},{bx:-45,by:-150,d:.08,s:20},{bx:45,by:-150,d:.04,s:16},{bx:-175,by:20,d:.15,s:18},{bx:175,by:20,d:.02,s:20},{bx:-100,by:-120,d:.09,s:22},{bx:100,by:-120,d:.14,s:24}];
                return positions.map((p,i) => (<div key={i} style={{ position:"fixed", top:"48%", left:"50%", fontSize:p.s||22, lineHeight:1, "--bx":`${p.bx}px`, "--by":`${p.by}px`, animation:`burstPop 1.8s cubic-bezier(.22,1,.36,1) ${p.d}s both`, pointerEvents:"none", zIndex:50 }}>{emojis[i % emojis.length]}</div>));
              })()}
              <div style={{ maxWidth:580, margin:"0 auto", padding:"0 24px" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:26 }}>
                  ...
                  <button className="btn btn-accent" onClick={handleSend} disabled={celebrating} style={{ background:cat.accent, opacity:celebrating?.7:1, transition:"opacity .2s" }}>{celebrating ? "✦ Sealing…" : "✦ Finalise Card"}</button>
                </div>
              </div>
            </div>
          )}
